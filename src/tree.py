...
class Tree:
    ...
    def requests(self):
        ...
        erp_data = Odoo(
            usr_data["url"], usr_data["db"], usr_data["username"], usr_data["password"]
        )
        return erp_data

    def ins_elements(
        self,
    ):  # Saca un diccionario con la cantidad de elementos de cada tipo
        ...
        if (
            request_read is not None
            and isinstance(request_read, list)
            and len(request_read) > 0
        ):
            elem_ids = set()
            ...
            for product in product_list:
                elem_req = self.erp.search(
                    "sale.order.line", "product_id.default_code", product
                )
                ...
                if elem_req is not None and isinstance(elem_req, list):
                    ...
                    if rep_elem:
                        ...
                        if ins_list_read is not None and isinstance(
                            ins_list_read, list
                        ):
    ...
    def router_data(self):  # Request a Odoo buscando datos del router del sistema
        ...
        if sys_read is not None and isinstance(sys_read, list) and len(sys_read) > 0:
            ...
            if sys_id is not None and isinstance(sys_id, list) and len(sys_id) > 0:
                router_request = self.erp.search(
                    "altatec.router", "sistema_id", sys_id[0]
                )
                router_read = self.erp.read(router_request)
                if (
                    router_read is not None
                    and isinstance(router_read, list)
                    and len(router_read) > 0
                ):
